...
from app.models.schemas import TokenRequest, TokenData, TokenResponse
...
def _decode(token: str) -> TokenData:
    try:
        ...
        # Map JWT 'sub' to schema 'userId'
        return TokenData(userId=data["sub"])
...
def require_user(path_user_id: str, token: TokenData = Depends(get_current_user)) -> TokenData:
    ...
    if token.userId != path_user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Token sub '{token.userId}' does not match requested userId '{path_user_id}'",
        )
    return token
